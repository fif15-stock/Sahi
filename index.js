...
async function scrapeNews() {
    try {
        ...
        await pageInstance.waitForSelector('[data-testid="buzz-news-item"]', { 
            timeout: 30000 
        });
        ...
        const newsItems = await pageInstance.evaluate(() => {
            ...
            const newsContainer = document.querySelector('[data-testid="buzz-news-container"]');
            
            if (!newsContainer) return items;

            // Get all news items
            const itemElements = newsContainer.querySelectorAll('[data-testid="buzz-news-item"]');
            
            itemElements.forEach(item => {
                const headlineEl = item.querySelector('h3');
