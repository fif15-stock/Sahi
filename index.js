...
const CONFIG = {
    ...
    groqRequestDelay: process.env.GROQ_REQUEST_DELAY ? parseInt(process.env.GROQ_REQUEST_DELAY) : 10000, 
    maxHtmlContentLength: process.env.MAX_HTML_CONTENT_LENGTH ? parseInt(process.env.MAX_HTML_CONTENT_LENGTH) : 500, 
    wpApiUrl: process.env.WP_API_URL || 'https://profitbooking.in/wp-json/scraper/v1/sahibuzz',
    googleSheet: {
...
class NewsProcessor {
    constructor() {
        ...
        this.maxRetries = 5;
        this.processedHeadlines = new Set();
    }
    ...
    async addToQueue(items) {
        ...
        const newItems = items.filter(item =>
            item &&
            item.headline &&
            !this.queue.some(existing => existing.headline === item.headline) &&
            !this.processedHeadlines.has(item.headline)
        );
        ...
        if (newItems.length > 0) {
            ...
            console.log('No new unique items to add to queue.');
        }
    }
    ...
    async processQueue() {
        ...
        try {
            console.log(`Processing headline: "${article.headline.substring(0, 50).replace(/\s+/g, ' ').trim()}..."`);

            const result = await this.analyzeWithAI(article);
            if (result) {
                await this.storeInWordPress(result);
                this.processedHeadlines.add(article.headline);
            } else {
    ...
    async analyzeWithAI(article) {
        ...
        const prompt = `Extract relevant NSE company from the following text and HTML. Output JSON: {"company_name":"string|null", "headline":"string|null", "description":"string|null", "nsc":"string|null"}. Prioritize the exact headline. If no relevant company, set "nsc" and all other fields to null.

Text: "${article.headline}${article.description ? "\\n\\n" + article.description : ""}"
HTML context: ${article.htmlContent.substring(0, CONFIG.maxHtmlContentLength)}`; 
    ...
    async storeInWordPress(data) {
        try {
            const requiredFields = ['description', 'headline', 'nsc'];
            if (!requiredFields.every(field => data[field])) {
                console.log('Skipping WordPress storage: Missing required fields (description, headline, nsc). Data:', data);
                return;
            }
...
async function scrapeNews() {
    try {
        ...
        const newsItems = await pageInstance.evaluate(() => {
            const items = [];
            const newsContainers = document.querySelectorAll('.relative.p-4.transition-colors.duration-200.group.bg-surface-neutral-l1-dark.rounded-lg');
            
            newsContainers.forEach(container => {
                try {
                    // Extract category and time
                    const metaContainer = container.querySelector('.flex.justify-between.items-start.mb-2');
                    const categoryEl = metaContainer?.querySelector('.text-sunrise-800');
                    const timeEl = metaContainer?.querySelector('.text-neutral-tertiary-dark');
                    
                    // Extract headline and description
                    const headlineEl = container.querySelector('h3.text-lg.font-semibold.mb-2.text-neutral-primary-dark');
                    const descriptionEl = container.querySelector('p.text-sm.text-neutral-secondary-dark');
                    
                    if (headlineEl) {
                        items.push({
                            category: categoryEl?.innerText?.trim() || 'General',
                    ...
                    console.error('Error processing one news item:', e);
                }
            });
        ...
        try {
            await pageInstance.screenshot({ path: 'scraping-error.png' });
            console.log('Saved screenshot as scraping-error.png for debugging');
        } catch (screenshotErr) {
