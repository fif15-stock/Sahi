...
async function scrapeNews() {
    try {
        ...
        await pageInstance.waitForSelector('.relative.p-4.transition-colors.duration-200.group.bg-surface-neutral-l1-dark.rounded-lg.cursor-pointer.hover\\:bg-surface-neutral-l2-dark', { timeout: 30000 });

        console.log('Extracting news items...');
        const newsItems = await pageInstance.evaluate(() => {
            const items = [];
            document.querySelectorAll('.relative.p-4.transition-colors.duration-200.group.bg-surface-neutral-l1-dark.rounded-lg.cursor-pointer.hover\\:bg-surface-neutral-l2-dark').forEach(item => {
                const headlineEl = item.querySelector('h3.text-lg.font-semibold.mb-2.text-neutral-primary-dark');
                const descriptionEl = item.querySelector('p.text-sm.text-neutral-secondary-dark');
        ...
        if (err.name === 'TimeoutError') {
            ...
            console.error('1. Website structure changed - please manually inspect `https://sahi.com/sahi-buzz` to verify selectors.');
